...
public class Query<TEntity, TDbContext> : IQuery<TEntity> where TEntity : class
{
    private int? _skip;
    private int? _take;
    ...
    public Query(IRepository<TEntity, TDbContext> repository) => _query = repository.Queryable();
    ...
    private IQuery<TEntity> Set(Action<Query<TEntity, TDbContext>> setParameter)
    {
        setParameter(this);
        return this;
    }
